      ...
      const addClient = async (e) => {
       e.preventDefault()
        ...
        const { data, error } = await supabase.from('clients').insert([
          { name:name, email:email, phone:phone,address:address,taxId:taxId,gstNo:gstNo,startdate:startdate,endDate:endDate,
            annualRevenue:annualRevenue,acType:acType,acNo:acNo,status:status,notes:notes,
           },
        ]);
   
        if (!name || !email || !phone  ||!address ||!taxId || !gstNo || !startdate || !endDate || !annualRevenue || !acType || !status 
            || !notes || !acNo
         ) {
...
<Card className="w-full sm:w-96 p-8 shadow-lg rounded-lg bg-white dark:bg-gray-800">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Add New Record</h2>
...
<div>
    <Label htmlFor="startNotes" className="text-gray-700 dark:text-white">Notes</Label>
        ...
        <div className="mt-6">
        <Button onClick={(e)=>addClient(e)}  className="w-full py-3 bg-blue-600 text-white rounded-md">Add Client</Button></div>
        </form>
